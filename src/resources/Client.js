...
export class Client {
	static async GET(params) {
		...
		try {
				...
				auth: params.auth,
					...
					if (e.response) {
						...
						switch (res.status) {
							...
							case 401:
								msg =
									'Usuario no autenticado o token vencido, favor de acceder nuevamente';
							...
							case 500:
								Utils.swalError(
									'Error de servidor por favor comunicarse con el soporte tecnico'
								);
								break;
							default:
								if (res.message) {
			...
			const res = await axios(requestParamas).catch(requestParamas.catch);
			if (res === undefined) {
				response = [];
			} else {
				if (res.data.success) {
	...
	static async POST(params) {
		...
		try {
			...
			const res = await axios(requestParams).catch(requestParams.catch);
			if (res === undefined) {
				response = {};
			} else {
				if (res.data.success) {
	...
	static async DOWNLOAD(params) {
		...
		await axios
			...
			.then(({ data }) => {
				...
				link.setAttribute(
					'download',
					'Cuponealo-' + this.formatDate(new Date()) + '.zip'
				); // any other extension
